<template>
  <div class="microdoneH5">
    <div>
      <span>账&nbsp;&nbsp;&nbsp;&nbsp;号：</span>
      <input
        type="text"
        value="tester"
        id="username2"
        name="username"
        class="de2 NoneInput90"
        maxlength="50"
        tabindex="1"
      />
    </div>
    <div>
      <span>登录密码：</span>
      <input
        type="text"
        readonly="readonly"
        id="kb1"
        name="input3"
        placeholder="请输入密码"
        class="default NoneInput90"
        maxlength="50"
        tabindex="3"
      />
    </div>
  </div>
</template>

<script>
console.log(1111,ENV_CONFIG.assetsUrl+"assets/send_mapping.jsp")
export default {
  name: "Home",
  components: {
    
  },
  mounted() {
    this.$nextTick(() => {
      var kb = new keyBoard({
        chaosMode: 0, // 混乱模式,0:无混乱;1:打开时乱一次;2:每输入一个字符乱一次,默认值0
        pressStatus: 1, // 按键状态,0:按下、抬起按键无变化;1:按下、抬起按键的颜色变化,默认值0
        svg: "svg", //svg图片的地址
        hasMap: 1, //是否使用映射层算法，1表示使用;0表示不使用
        license:
          "",
      });
      //映射表指定，也可以通过passGuard对象的mappurl属性指定后端url进行下发
      passGuard.mapArr =
        "";

      var passGuard1 = new passGuard({
        mappurl: ENV_CONFIG.assetsUrl+"assets/send_mapping.jsp", //映射表下发地址，当kb对象的hasMap为1时生效，也可以通过passGuard.mapArr直接指定映射表
        maxLength: 12, // 允许输入的最大输入长度
        regExp1: "[\\S\\s]", // 输入过程限制的正则
        regExp2: "^[0-9]{6,12}$", // 输入完毕后待验证的正则，与函数getValid()对应
        displayMode: 0, // 显示形式,0:星号;1:明文,默认值0
        callBack: cb1, //已输入的密码匹配正则regExp2成功时回调，键盘隐藏时回调此函数
        errorCallBack: cb2, //已输入的密码匹配正则regExp2失败时回调，键盘隐藏时回调此函数
        focus: inputFocus1, //H5键盘获取焦点回调此函数
        blur: inputBlur1, //H5键盘失去焦点回调此函数
        overHeight: "0", //远离输入框的距离（一般键盘都是在输入框的正下方，接口不写时默认为0）
        sm2KeyHex:
          "1093A047C5CBF48283EC7A210703F3FF9EA4448DC15D56B4CD82FCB27DAD2D45F2BB0BF953BCEBB635D9D34E473ECEFC9F25880EB1669F94DE050A29AC86F308",
        //RSA公钥
        rsaPublicKey:
          "", // rsa公钥
      });
      window.passGuard1 = passGuard1;
      function cb1() {
        console.log("匹配正则2成功回调");
      }

      function cb2() {
        console.log("匹配正则2失败回调");
      }
      //H5键盘获取焦点回调此函数
      function inputFocus1() {
        $1.css("#kb1", { border: "1px solid red" });
      }
      //H5键盘失去焦点回调此函数
      function inputBlur1() {
        $1.css("#kb1", { border: "1px solid #000" });
      }
      // --------------------------------

      window.onload = function() {
        //清空密码键盘input框的值，防止缓存

        setTimeout(function() {
          //绘制密码键盘html标签
          kb.generate();
        }, 100);
      };
      //初始化密码卫士,绑定键盘对象。数字参数：0代表全键盘，1代表数字键盘
      passGuard1.generate("kb1", kb, "passGuard1", 0);
    });
  },
};
</script>
